...
pub struct SandboxConfig {
    roots: Vec<PathBuf>,          // original paths (for display)
    canonical_roots: Vec<PathBuf>, // canonicalized (for validation)
}
```
...
- **`add_roots<I, P>(&mut self, roots: I) -> Result<(), DocumentError>`**
...
- **`async resolve_path_with_permission(&self, policy: &PermissionPolicy, tool_name: &str, description: &str, user_path: &Path) -> Result<PathBuf, DocumentError>`**
  Resolves `user_path`, consulting the permission gate **only** when the path falls outside the sandbox. In-sandbox paths are auto-allowed without invoking the gate. Out-of-sandbox paths check via `check_permission`; on `Allow` the path is resolved with `resolve_path_unchecked`.
...
Returns the original (non-canonicalized) root that contains `path`, or `None` if the path does not fall under any root. Uses canonical roots for comparison with a fallback to non-canonical comparison for non-existent paths.
