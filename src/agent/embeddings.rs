	...
	M: EmbeddingModel,
{
	...
	/// each document.
	pub async fn embed_documents<T, I>(&self, documents: I) -> Result<Vec<(T, OneOrMany<Embedding>)>>
	where
		T: Embed,
...
pub fn service_from_client<C>(client: &C, model: impl Into<String>) -> EmbeddingService<C::EmbeddingModel>
where
	C: EmbeddingsClient,
{
