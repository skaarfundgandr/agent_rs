...
use rig::tool::{rmcp::McpTool as RigMcpTool, ToolDyn, ToolError};
...
impl McpRegistry {
	...
	pub async fn connect(&self) -> Result<McpRegistryRuntime> {
		...
		for resolved in self.resolved_servers()? {
			...
			for tool in &tools {
				if let Some(previous_server) = seen_tool_names.insert(tool.tool_name.clone(), server.name.clone()) {
					bail!(
...
async fn connect_stdio_server(name: String, spec: McpStdioTransportSpec) -> Result<ConnectedMcpServer> {
...
async fn collect_registered_tools(
	...
	for tool in tools {
		...
		if !seen_local_names.insert(tool_name.clone()) {
			bail!("MCP server `{name}` returned duplicate tool `{}`", tool_name);
		}
